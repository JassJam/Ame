package("ame.mimalloc")
    ...
    add_configs("skipcollectonexit", {description = "Skip collecting memory on program exit", default = false, type = "boolean"})
    ...
    on_install("macosx", "windows", "linux", "android", "mingw", function (package)
        ...
        table.insert(configs, "-DMI_DEBUG_FULL=" .. (package:config("debug") and "ON" or "OFF"))
        table.insert(configs, "-DMI_SKIP_COLLECT_ON_EXIT=" .. (package:config("skipcollectonexit") and "ON" or "OFF"))
