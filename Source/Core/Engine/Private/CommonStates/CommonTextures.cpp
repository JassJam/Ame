...
{
    [[nodiscard]]
    static std::vector<std::uint8_t> DecompressTextureData(std::span<const std::uint8_t> data)
    {
    ...
    [[nodiscard]]
    static Ptr<Dg::ITexture> CreateTexture(IRhiDevice*                      rhiDevice,
...
#define COMMON_TEXTURES_111(Name)                                                                  \
    ...
    {                                                                                              \
        Dg::TextureSubResData subresources[] = { { s_##Name##Texture,                              \
                                                   sizeof(s_##Name##Texture) } };                  \
        m_DefaultTextures[index]             = CreateTexture(m_RhiDevice,                          \
                                                 Dg::RESOURCE_DIM_TEX_1D,              \
                                                 #Name "1D",                           \
                                                             { 1, 1, 1 },                          \
                                                 subresources);                        \
        break;                                                                                     \
    ...
    Dg::ITexture* CommonTextures::GetCheckboardTexture(const Math::Vector2U& size,
                                                       const Math::Vector2U& slices)
    {
        ...
        if (!texture)
        {
            ...
            Dg::GenerateCheckerBoardPattern(size.x(),
                                            size.y(),
            ...
            texture = CreateTexture(m_RhiDevice,
                                    Dg::RESOURCE_DIM_TEX_2D,
                                    "Checkboard",
                                    { size.x(), size.y(), 1 },
                                    subresources);
    ...
    Dg::ITexture* CommonTextures::GetDevTexture(const Math::Vector2U& size)
    {
        ...
        if (!texture)
        {
            ...
            auto imageMemory =
                ImageStorage::Load(std::bit_cast<std::byte*>(imageData.data()), imageData.size());
