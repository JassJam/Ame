...
namespace Ame::Gfx
{
    ...
    void EntityStorage::UpdateFrameData(const CameraFrameDataUpdateDesc& updateDesc)
    {
        auto renderContext = m_RhiDevice->GetImmediateContext();

        CameraFrameData frameData{ .World = updateDesc.World,
    ...
    void EntityStorage::UpdateLightInstances(std::span<const uint32_t> lightIds)
    {
    ...
    void EntityStorage::UploadAllResource(Rg::Graph& cameraGraph)
    {
