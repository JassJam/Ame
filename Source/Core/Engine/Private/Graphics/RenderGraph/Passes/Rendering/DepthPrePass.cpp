...
namespace Ame::Gfx
{
    ...
    void DepthPrePass::CreateResourcesOnce(const Rg::ResourceStorage& storage)
    {
        ...
        Rhi::MaterialRenderState renderState{
            ...
            .Links{ .Sources{ { Dg::SHADER_TYPE_VERTEX,
                                Rhi::DepthPrepass_VertexShader().GetCreateInfo() } },
    ...
    void DepthPrePass::OnBuild(Rg::Resolver& resolver)
    {
        ...
        m_PassData.DepthView = resolver.WriteTexture(c_RGDepthImage,
    ...
    void DepthPrePass::OnExecute(const Rg::ResourceStorage& storage,
                                 Dg::IDeviceContext*        deviceContext)
    {
        auto dsv = storage.GetTextureView(m_PassData.DepthView);

        deviceContext->SetRenderTargets(0,
                                        nullptr,
                                        dsv,
                                        Dg::RESOURCE_STATE_TRANSITION_MODE_TRANSITION);
        deviceContext->ClearDepthStencil(dsv,
                                         Dg::CLEAR_DEPTH_FLAG,
                                         1.f,
                                         0,
                                         Dg::RESOURCE_STATE_TRANSITION_MODE_TRANSITION);
