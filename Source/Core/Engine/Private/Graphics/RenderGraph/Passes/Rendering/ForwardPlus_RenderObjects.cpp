...
namespace Ame::Gfx
{
    ...
    void ForwardPlus_RenderObjectsPass::BindResourceOnce(const Rg::ResourceStorage& storage)
    {
        if (m_PassData)
        {
            return;
    ...
    void ForwardPlus_RenderObjectsPass::OnExecute(const Rg::ResourceStorage& storage, Dg::IDeviceContext* deviceContext)
    {
        ...
        StdRenderObjectDesc drawProps{
            m_World, m_PassData.Srbs, m_PassData.LightSrb, deviceContext, m_Technique,
        };
