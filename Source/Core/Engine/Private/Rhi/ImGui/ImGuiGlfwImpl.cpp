...
//  ImGuiBackendFlags_HasMouseCursors flag
//  + honor ImGuiConfigFlags_NoMouseCursorChange flag. 2018-02-20: Inputs: Added support for mouse
//  cursors (ImGui::GetMouseCursor() value, passed to glfwSetCursor()). 2018-02-06: Misc: Removed
...
//  2016-10-15: Misc: Added a void* user_data parameter to Clipboard function handlers.
    ...
    #define GLFW_HAS_PER_MONITOR_DPI                                                               \
        ...
        defined(                                                                                   \
            __SWITCH__) // no Vulkan support in GLFW for Emscripten or homebrew Nintendo Switch
    ...
    #define GLFW_HAS_OSX_WINDOW_POS_FIX                                                            \
        (GLFW_VERSION_COMBINED >=                                                                  \
         3301)                     // 3.3.1+ Fixed: Resizing window repositions it on MacOS #1553
                                   ...
                                   // (3.4 define) and 2019-11-29 (cursors defines) // FIXME: Remove
...
{
    bool chain_callback = ImGui_ImplGlfw_ShouldChainCallback(window);
...
void ImGui_ImplGlfw_InstallCallbacks(GLFWwindow* window)
{
    ...
    bd->UserCallbackWindowFocusId =
        GlfwHooks::Get().Install_WindowFocus(window, ImGui_ImplGlfw_WindowFocusCallback);
    bd->UserCallbackCursorEnterId =
        GlfwHooks::Get().Install_CursorEnter(window, ImGui_ImplGlfw_CursorEnterCallback);
    bd->UserCallbackCursorPosId =
        GlfwHooks::Get().Install_CursorPos(window, ImGui_ImplGlfw_CursorPosCallback);
    bd->UserCallbackMousebuttonId =
        GlfwHooks::Get().Install_MouseButton(window, ImGui_ImplGlfw_MouseButtonCallback);
    bd->UserCallbackScrollId =
        GlfwHooks::Get().Install_Scroll(window, ImGui_ImplGlfw_ScrollCallback);
    ...
    bd->UserCallbackMonitorId = GlfwHooks::Get().Install_Monitor(
        [window](GLFWmonitor* monitor, int event)
...
static void ImGui_ImplGlfw_UpdateMouseData()
{
    ...
    #else
            // We cannot use bd->MouseWindow maintained from CursorEnter/Leave callbacks, because it
            // is locked to the window capturing mouse.
    #endif
    }
...
static void ImGui_ImplGlfw_UpdateMonitors()
{
    ...
    if (monitors_count ==
        0) // Preserve existing monitor list if there are none. Happens on macOS sleeping (#5683)
        return;
...
void ImGui_ImplGlfw_InstallEmscriptenCallbacks(GLFWwindow* window, const char* canvas_selector)
{
    GLFWwindow* w = (GLFWwindow*) (EM_ASM_INT(
        { return Module.glfwGetWindow(UTF8ToString($0)); },
        canvas_selector));
...
static void ImGui_ImplGlfw_CreateWindow(ImGuiViewport* viewport)
{
    ...
    using Ame::Window::GlfwHooks;
    vd->WindowFocusCallbackId =
        GlfwHooks::Get().Install_WindowFocus(vd->Window, ImGui_ImplGlfw_WindowFocusCallback);
    vd->CursorEnterCallbackId =
        GlfwHooks::Get().Install_CursorEnter(vd->Window, ImGui_ImplGlfw_CursorEnterCallback);
    vd->CursorPosCallbackId =
        GlfwHooks::Get().Install_CursorPos(vd->Window, ImGui_ImplGlfw_CursorPosCallback);
    vd->MouseButtonCallbackId =
        GlfwHooks::Get().Install_MouseButton(vd->Window, ImGui_ImplGlfw_MouseButtonCallback);
    vd->ScrollCallbackId =
        GlfwHooks::Get().Install_Scroll(vd->Window, ImGui_ImplGlfw_ScrollCallback);
    ...
    vd->WindowCloseCallbackId =
        GlfwHooks::Get().Install_WindowClose(vd->Window, ImGui_ImplGlfw_WindowCloseCallback);
    vd->WindowPosCallbackId =
        GlfwHooks::Get().Install_WindowPos(vd->Window, ImGui_ImplGlfw_WindowPosCallback);
    vd->WindowSizeCallbackId =
        GlfwHooks::Get().Install_WindowSize(vd->Window, ImGui_ImplGlfw_WindowSizeCallback);
...
static void ImGui_ImplGlfw_SwapBuffers(ImGuiViewport* viewport, void*)
{
    ...
    //--------------------------------------------------------------------------------------------------------
    // Vulkan support (the Vulkan renderer needs to call a platform-side support function to create
    // the surface)
    //--------------------------------------------------------------------------------------------------------
