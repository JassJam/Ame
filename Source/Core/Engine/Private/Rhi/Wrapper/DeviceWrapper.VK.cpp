...
namespace Ame::Rhi
{
    ...
    struct DeviceCreateTraitsVk
    {
        ...
        static diligent_create_info GetCreateInfo(const DeviceCreateDesc&,
                                                  const create_struct_type& createDescDev)
        {
            static VkAllocationCallbacks allocation{
                ...
                .pfnReallocation =
                    [](void*, void* ptr, size_t size, size_t, VkSystemAllocationScope) -> void*
            ...
            createInfo.InstanceLayerCount =
                static_cast<uint32_t>(createDescDev.RequiredLayerExtensions.size());
            ...
            createInfo.DeviceExtensionCount =
                static_cast<uint32_t>(createDescDev.RequiredLayerExtensions.size());
            ...
            createInfo.IgnoreDebugMessageCount =
                static_cast<uint32_t>(createDescDev.IgnoredDebugMessageNames.size());
            createInfo.ppIgnoreDebugMessageNames = createDescDev.IgnoredDebugMessageNames.data();

            for (auto [pool, desc] : { std::pair{ &createInfo.MainDescriptorPoolSize,
                                                  &createDescDev.MainDescriptorPool },
                                       std::pair{ &createInfo.DynamicDescriptorPoolSize,
                                                  &createDescDev.DynamicDescriptorPool } })
            {
                GetOrDefault(pool->MaxDescriptorSets, desc->MaxDescriptorSets);
                GetOrDefault(pool->NumSeparateSamplerDescriptors,
                             desc->NumSeparateSamplerDescriptors);
                GetOrDefault(pool->NumCombinedSamplerDescriptors,
                             desc->NumCombinedSamplerDescriptors);
                ...
                GetOrDefault(pool->NumUniformTexelBufferDescriptors,
                             desc->NumUniformTexelBufferDescriptors);
                GetOrDefault(pool->NumStorageTexelBufferDescriptors,
                             desc->NumStorageTexelBufferDescriptors);
                GetOrDefault(pool->NumInputAttachmentDescriptors,
                             desc->NumInputAttachmentDescriptors);
                GetOrDefault(pool->NumAccelStructDescriptors, desc->NumAccelStructDescriptors);
            }

            GetOrDefault(createInfo.DeviceLocalMemoryPageSize,
                         createDescDev.DeviceLocalMemoryPageSize);
            GetOrDefault(createInfo.HostVisibleMemoryPageSize,
                         createDescDev.HostVisibleMemoryPageSize);
            GetOrDefault(createInfo.DeviceLocalMemoryReserveSize,
                         createDescDev.DeviceLocalMemoryReserveSize);
            GetOrDefault(createInfo.HostVisibleMemoryReserveSize,
                         createDescDev.HostVisibleMemoryReserveSize);
        ...
        {
            factory->CreateSwapChainVk(renderDevice,
                                       deviceContext,
                                       swapchainDesc,
                                       nativeWindow,
                                       swapchain);
