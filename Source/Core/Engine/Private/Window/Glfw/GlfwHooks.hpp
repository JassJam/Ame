...
private:                                                                                           \
    ...
    static void GLFW##name##Callback(Args... args)                                                 \
    {                                                                                              \
        auto& self = Get();                                                                        \
...
public:                                                                                            \
    ...
    [[nodiscard]]                                                                                  \
    uint32_t Install_##name(FnTy&& callback)                                                       \
    {                                                                                              \
        auto id = m_IdCounter++;                                                                   \
...
private:                                                                                           \
    template<typename... Args>                                                                     \
    static void GLFW##name##Callback(GLFWwindow* window, Args... args)                             \
    {                                                                                              \
        auto& self = Get();                                                                        \
        AME_INSTALL_GLFW_CALLBACK_PROLOG;                                                          \
...
public:                                                                                            \
    template<typename FnTy>                                                                        \
    [[nodiscard]]                                                                                  \
    uint32_t Install_##name(GLFWwindow* window, FnTy&& callback)                                   \
    {                                                                                              \
        auto id = m_IdCounter++;                                                                   \
...
{
    /// <summary>
    /// Since glfw is a singleton, we need to have a way to properly manage and dispatch callbacks
    /// between the hooks. All operations must be called inside GlfwContext
    /// </summary>
    class GlfwHooks
    {
    public:
    ...
    public:
        ...
        [[nodiscard]]
        bool IsCallbackInProgress() const noexcept
        {
        ...
        /// </summary>
        [[nodiscard]]
        static GlfwHooks& Get();

        //

    public:
        AME_INSTALL_GLFW_CALLBACKS(GLFWerrorfun, glfwSetErrorCallback, Error, int, const char*);
        AME_INSTALL_GLFW_CALLBACKS(GLFWmonitorfun,
                                   glfwSetMonitorCallback,
                                   Monitor,
                                   GLFWmonitor*,
                                   int);

        AME_INSTALL_GLFW_CALLBACKS_WINDOW(GLFWwindowposfun,
                                          glfwSetWindowPosCallback,
                                          WindowPos,
                                          int,
                                          int);
        AME_INSTALL_GLFW_CALLBACKS_WINDOW(GLFWtitlebarhittestfun,
                                          glfwSetTitlebarHitTestCallback,
                                          TitlebarHitTest,
                                          int,
                                          int,
                                          int*);
        AME_INSTALL_GLFW_CALLBACKS_WINDOW(GLFWwindowsizefun,
                                          glfwSetWindowSizeCallback,
                                          WindowSize,
                                          int,
                                          int);
        AME_INSTALL_GLFW_CALLBACKS_WINDOW(GLFWwindowclosefun,
                                          glfwSetWindowCloseCallback,
                                          WindowClose);
        AME_INSTALL_GLFW_CALLBACKS_WINDOW(GLFWwindowrefreshfun,
                                          glfwSetWindowRefreshCallback,
                                          WindowRefresh);
        AME_INSTALL_GLFW_CALLBACKS_WINDOW(GLFWwindowfocusfun,
                                          glfwSetWindowFocusCallback,
                                          WindowFocus,
                                          int);
        AME_INSTALL_GLFW_CALLBACKS_WINDOW(GLFWwindowiconifyfun,
                                          glfwSetWindowIconifyCallback,
                                          WindowIconified,
                                          int);
        AME_INSTALL_GLFW_CALLBACKS_WINDOW(GLFWwindowmaximizefun,
                                          glfwSetWindowMaximizeCallback,
                                          WindowMaximized,
                                          int);
        AME_INSTALL_GLFW_CALLBACKS_WINDOW(GLFWframebuffersizefun,
                                          glfwSetFramebufferSizeCallback,
                                          WindowFrameBufferSize,
                                          int,
                                          int);
        AME_INSTALL_GLFW_CALLBACKS_WINDOW(GLFWwindowcontentscalefun,
        ...
        AME_INSTALL_GLFW_CALLBACKS_WINDOW(GLFWcharmodsfun,
                                          glfwSetCharModsCallback,
                                          CharMods,
                                          unsigned int,
                                          int);
        AME_INSTALL_GLFW_CALLBACKS_WINDOW(GLFWmousebuttonfun,
                                          glfwSetMouseButtonCallback,
                                          MouseButton,
                                          int,
                                          int,
                                          int);
        AME_INSTALL_GLFW_CALLBACKS_WINDOW(GLFWcursorposfun,
                                          glfwSetCursorPosCallback,
                                          CursorPos,
                                          double,
                                          double);
        AME_INSTALL_GLFW_CALLBACKS_WINDOW(GLFWcursorenterfun,
                                          glfwSetCursorEnterCallback,
                                          CursorEnter,
                                          int);
        AME_INSTALL_GLFW_CALLBACKS_WINDOW(GLFWscrollfun,
                                          glfwSetScrollCallback,
                                          Scroll,
                                          double,
                                          double);
        AME_INSTALL_GLFW_CALLBACKS_WINDOW(GLFWdropfun,
                                          glfwSetDropCallback,
                                          DropPath,
                                          int,
                                          const char**);
