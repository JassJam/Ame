...
namespace Ame::Geometry
{
    ...
    public:
        [[nodiscard]]
        std::span<Math::Plane> GetPlanes() noexcept
        {
            return Planes;
        }

        [[nodiscard]]
        std::span<const Math::Plane> GetPlanes() const noexcept
        {
            return Planes;
        }
    ...
    public:
        [[nodiscard]]
        constexpr Math::Plane& operator[](Type type) noexcept
        {
            return Planes[static_cast<size_t>(type)];
        }

        [[nodiscard]]
        constexpr const Math::Plane& operator[](Type type) const noexcept
        {
            return Planes[static_cast<size_t>(type)];
        }
    };
