...
namespace imcxx
{
    ...
    public:
              ...
              ImGuiInputTextFlags flags       = 0)
        {
            ...
            static_assert(type != ImGuiDataType_COUNT,
                          "Invalid type was passed to 'ImGui::InputScalar()'");
#if _HAS_CXX20
              ...
              ImGuiInputTextFlags flags       = 0)
        {
            static constexpr ImGuiDataType type = impl::to_imdatatype<_Ty>();
            static_assert(type != ImGuiDataType_COUNT,
                          "Invalid type was passed to 'ImGui::InputScalarN()'");
#if _HAS_CXX20
...
#if _HAS_CXX20
            m_Result._Value = ImGui::InputScalarN(impl::get_string(label),
                                                  type,
        ...
        /// </summary>
        template<size_t _ViewSize, typename _StrTy, typename _Ty, size_t _Size>
        input(array<_ViewSize> arr_size,
              const _StrTy&    label,
              _Ty (&v)[_Size],
              _Ty              v_slow_step = {},
              _Ty              v_fast_step = {},
        ...
        /// <summary>
        /// Calls 'ImGui::InputScalarN' with a smaller or equal to array size 'v', overload for
        /// 'ImVec2'
        /// </summary>
...
#if _HAS_CXX20
              input(label,
                    std::bit_cast<std::array<float, 2>*>(&v),
                    v_slow_step,
                    v_fast_step,
                    format,
                    flags)
#else
              input(label,
                    reinterpret_cast<std::array<float, 2>*>(&v),
                    v_slow_step,
                    v_fast_step,
                    format,
                    flags)
...
#if _HAS_CXX20
              input(label,
                    std::bit_cast<std::array<float, 4>*>(&v),
                    v_slow_step,
                    v_fast_step,
                    format,
                    flags)
#else
              input(label,
                    reinterpret_cast<std::array<float, 4>*>(&v),
                    v_slow_step,
                    v_fast_step,
                    format,
                    flags)
        ...
        {
            if constexpr (std::is_same_v<std::remove_pointer_t<std::decay_t<_InputTy>>,
                                         std::string>)
            {
                ...
                m_Result._Value = ImGui::InputText(impl::get_string(label),
                                                   p_input->data(),
                                                   p_input->capacity() + 1,
        ...
        /// <summary>
        /// for restricted size of a text buffer, we'll be passing input_size before input buffer to
        /// avoid flags ambiguity
        /// </summary>
              ...
              void*                  user_data = nullptr)
        {
                                         ...
                                         std::string>)
            {
                ...
                input_reforward_data data{ p_input, callback, user_data };
                m_Result._Value =
                    ImGui::InputTextMultiline(impl::get_string(label),
                                              input,
                                              size,
                                              p_input->data(),
                                              p_input->capacity() + 1,
                ...
                m_Result._Value = ImGui::InputTextMultiline(impl::get_string(label),
                                                            input,
                                                            std::extent_v<_InputTy>,
                                                            size,
                                                            flags,
        ...
        input(multiline,
              ...
              ImGuiInputTextCallback callback  = nullptr,
              void*                  user_data = nullptr)
            : scope_wrap(ImGui::InputTextMultiline(label,
                                                   input,
                                                   input_size,
                                                   size,
                                                   flags,
              ...
              void*                  user_data = nullptr)
        {
                                         ...
                                         std::string>)
            {
                ...
                input_reforward_data data{ p_input, callback, user_data };
                m_Result._Value =
                    ImGui::InputTextWithHint(impl::get_string(label),
            ...
            {
                m_Result._Value = ImGui::InputTextWithHint(impl::get_string(label),
                                                           impl::get_string(hint),
                                                           input,
                                                           std::extent_v<_InputTy>,
                                                           flags,
        ...
        input(hint,
              ...
              ImGuiInputTextCallback callback  = nullptr,
              void*                  user_data = nullptr)
            : scope_wrap(ImGui::InputTextWithHint(label,
                                                  hint,
                                                  input,
                                                  input_size,
                                                  flags,
