...
namespace imcxx
{
    ...
    public:
        ...
        tree_node(const _StrTy& label, ImGuiTreeNodeFlags flags = 0)
            : scope_wrap(
                  ImGui::TreeNodeEx(std::is_same_v<void*, _StrTy> ? label : impl::get_string(label),
                                    flags)),
            ...
            : m_ShouldPop(should_tree_pop(flags))
        {
            if constexpr (std::is_same_v<_StrTy, void*>)
                ...
                m_Result._Value =
                    ImGui::TreeNodeExV(impl::get_string(label), 0, impl::get_string(fmt), arg_list);
        }
            ...
            : m_ShouldPop(should_tree_pop(flags))
        {
            if constexpr (std::is_same_v<_StrTy, void*> || std::is_same_v<_StrTy, const void*>)
                m_Result._Value = ImGui::TreeNodeEx(label,
                                                    flags,
                                                    impl::get_string(fmt),
                                                    std::forward<_Args>(args)...);
            else
                m_Result._Value = ImGui::TreeNodeEx(impl::get_string(label),
                                                    flags,
                                                    impl::get_string(fmt),
                                                    std::forward<_Args>(args)...);
        }
        ...
        template<typename _StrTy>
        tree_node(push, const _StrTy& label) : m_ShouldPop(true)
        {
