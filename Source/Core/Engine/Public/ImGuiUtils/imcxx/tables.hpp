...
namespace imcxx
{
    ...
    ///      row if needed.
    ///    - IMPORTANT: Comparatively to the old Columns() API, we need to call TableNextColumn()
    ...
    public:
        ...
        table(const _StrTy&   name,
              int             column,
              ImGuiTableFlags flags       = 0,
              const ImVec2&   outer_size  = {},
              float           inner_width = 0.0f)
            : scope_wrap(
                  ImGui::BeginTable(impl::get_string(name), column, flags, outer_size, inner_width))
        {
        }
        ...
        void nextrow_and_invoke(_InvokeTy&& fn, _Args&&... args) noexcept(
        ...
        /// </summary>
        template<typename _InvokeTy, typename... _Args>
        void nextcol_and_invoke(_InvokeTy&& fn, _Args&&... args) noexcept(
            std::is_nothrow_invocable_v<_InvokeTy, _Args...>)
        {
            if (next_column())
                fn(std::forward<_Args>(args)...);
        ...
        [[nodiscard]]
        column current_column() const;

        [[nodiscard]]
        column get_column(int column_n) const;
        ...
        /// change the color of a cell, row, or column. See ImGuiTableBgTarget_ flags for details
        /// </summary>
        void set_color(ImGuiTableBgTarget          target,
        ...
        /// sorting.
        /// - When 'SpecsDirty == true' you should sort your data. It will be true when sorting
        /// specs have changed
        ///   since last call, or the first time. Make sure to set 'SpecsDirty = false' after
        ///   sorting, else you may wastefully sort your data every frame!
        /// - Lifetime: don't hold on this pointer over multiple frames or past any subsequent call
        /// to BeginTable().
        /// </summary>
        [[nodiscard]]
        ImGuiTableSortSpecs* specs() const
        {
    ...
    private:
        ...
        void table_setup_impl(const _Ty& arg, const _Args&... rest)
        {
            if constexpr (std::is_same_v<setup_info, _Ty>)
                ImGui::TableSetupColumn(arg.label,
                                        arg.flags,
                                        arg.init_width_or_weight,
                                        arg.user_id);
            else
    ...
    public:
        ...
        /// <summary>
        /// change user accessible enabled/disabled state of a column. Set to false to hide the
        ...
        /// </summary>
        [[nodiscard]]
        void set_state(bool enable) const
        {
