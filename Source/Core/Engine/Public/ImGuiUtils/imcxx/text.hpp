...
namespace imcxx
{
    ...
    public:
        ...
        /// <summary>
        ///  text without formatting. Roughly equivalent to Text("%s", text) but: A) doesn't require
        ///  null terminated string if 'text_end' is specified, B) it's faster, no memory copy is
        ///  done, no buffer size limits, recommended for long chunks of text.
        /// </summary>
        ...
        text(const char (&str)[_Size]) : text(no_format{}, str, str + _Size)
        {
        }
        ...
        /// <summary>
        /// shortcut for PushTextWrapPos(0.0f); Text(fmt, ...); PopTextWrapPos();. Note that this
        ...
        {
            bool need_backup = (ImGui::GetCurrentWindow()->DC.TextWrapPos <
        ...
        /// </summary>
        template<size_t _Size>
        text(wrapped, const char (&str)[_Size]) : text(wrapped{}, no_format{}, str, str + _Size)
        {
        }
