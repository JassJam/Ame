...
namespace imcxx
{
    ...
    ///    corresponding BeginXXX function returned true. Begin and BeginChild are the only odd ones
    ///    out. Will be fixed in a future update.]
    ...
    /// size / <0.0f: use remaining window size minus abs(size) / Each axis can use a different
    /// mode, e.g. ImVec2(0,400).
    /// - BeginChild() returns false to indicate the window is collapsed or fully clipped, so you
    ...
    public:
        ...
        child(const _StrTy&    str_id,
              _VecTy           size        = {},
              ImGuiChildFlags  child_flags = 0,
              ImGuiWindowFlags flags       = 0)
            : scope_wrap(ImGui::BeginChild(impl::get_string(str_id),
                                           impl::to_imvec2(size),
                                           child_flags,
                                           flags))
        {
        ...
        child(frame, ImGuiID id, const _VecTy& size = {}, ImGuiWindowFlags flags = 0)
            : scope_wrap(
                  ImGui::BeginChild(id, impl::to_imvec2(size), ImGuiChildFlags_FrameStyle, flags))
        {
        }
