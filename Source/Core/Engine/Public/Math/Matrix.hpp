...
namespace Ame::Math
{
    ...
    public:
        ...
        constexpr Matrix4x4(const Vector4& row1,
                            const Vector4& row2,
                            const Vector4& row3,
                            const Vector4& row4)
    ...
    public:
        ...
        [[nodiscard]]
        static Matrix4x4 RotationRollPitchYaw(const Vector3& vec);

        [[nodiscard]]
        static Matrix4x4 RotationQuaternion(const Quaternion& quat);

        [[nodiscard]]
        static Matrix4x4 RotationNormal(const Vector3& axis, float angle);

        [[nodiscard]]
        static Matrix4x4 RotationAxis(const Vector3& axis, float angle);

        [[nodiscard]]
        static Matrix4x4 LookAt(const Vector3& eyePosition,
                                const Vector3& focusPosition,
                                const Vector3& upDirection);

        [[nodiscard]]
        static Matrix4x4 LookTo(const Vector3& eyePosition,
        ...
        [[nodiscard]]
        static Matrix4x4 PerspectiveFov(float fovAngleY,
                                        float aspectRatio,
                                        float nearZ,
                                        float farZ);

        [[nodiscard]]
        static Matrix4x4 Orthographic(float viewWidth, float viewHeight, float nearZ, float farZ);

        [[nodiscard]]
        static Matrix4x4 SRT(const Matrix4x4& scale,
                             const Matrix4x4& rotation,
                             const Matrix4x4& translation);

    public:
        [[nodiscard]]
        Vector2 DoTransform(const Vector2& vec) const;
        [[nodiscard]]
        Vector2 DoTransformCoord(const Vector2& vec) const;
        [[nodiscard]]
        Vector2 DoTransformNormal(const Vector2& vec) const;

        [[nodiscard]]
        Vector3 DoTransform(const Vector3& vec) const;
        [[nodiscard]]
        Vector3 DoTransformCoord(const Vector3& vec) const;
        [[nodiscard]]
        Vector3 DoTransformNormal(const Vector3& vec) const;

        [[nodiscard]]
        Vector4 DoTransform(const Vector4& vec) const;
        [[nodiscard]]
        Plane DoTransform(const Plane& plane) const;
        ...
        [[nodiscard]]
        constexpr Vector4& operator()(size_t row) noexcept
        {
            return m_Data[row];
        }
        ...
        [[nodiscard]]
        constexpr const Vector4& operator()(size_t row) const noexcept
        {
            return m_Data[row];
        }
    ...
    public:
        [[nodiscard]]
        std::tuple<Vector4, Quaternion, Vector3> GetPositionRotationScale() const;
    ...
    private:
        [[nodiscard]]
        static constexpr Matrix4x4 _Identity()
        {
            Matrix4x4 Mat;
    ...
    public:
        ...
        [[nodiscard]]
        static Matrix3x3 RotationRollPitchYaw(const Vector3& vec);

        [[nodiscard]]
        static Matrix3x3 RotationQuaternion(const Quaternion& quat);

        [[nodiscard]]
        static Matrix3x3 RotationNormal(const Vector3& axis, float angle);

        [[nodiscard]]
        static Matrix3x3 RotationAxis(const Vector3& axis, float angle);

        [[nodiscard]]
        static Matrix3x3 SR(const Matrix3x3& scale, const Matrix3x3& rotation);
    ...
    public:
        ...
        [[nodiscard]]
        constexpr Vector3& operator()(size_t row) noexcept
        {
            return m_Data[row];
        }
        ...
        [[nodiscard]]
        constexpr const Vector3& operator()(size_t row) const noexcept
        {
            return m_Data[row];
        }
    ...
    public:
        [[nodiscard]]
        std::pair<Quaternion, Vector3> GetRotationScale() const;
    ...
    struct Matrix4x4::Constants
    {
        ...
        static constexpr Matrix4x4 Identity{ Vector4{ 1.f, 0.f, 0.f, 0.f },
    ...
    struct Matrix3x3::Constants
    {
        ...
        static constexpr Matrix3x3 Identity{ Vector3{ 1.f, 0.f, 0.f },
                                             Vector3{ 0.f, 1.f, 0.f },
                                             Vector3{ 0.f, 0.f, 1.f } };
    };
