...
#define AME_VECTOR_IMPL_MATH_OP(VecType, Symbol)                                                   \
    ...
    [[nodiscard]]                                                                                  \
    VecType operator##Symbol(const VecType& other) const noexcept                                  \
    {                                                                                              \
        VecType Tmp   = *this;                                                                     \
        Tmp##Symbol## = other;                                                                     \
        return Tmp;                                                                                \
    ...
    [[nodiscard]]                                                                                  \
    VecType operator##Symbol(value_type val) const noexcept                                        \
    {                                                                                              \
        VecType Tmp   = *this;                                                                     \
        Tmp##Symbol## = val;                                                                       \
        return Tmp;                                                                                \
...
#define AME_VECTOR_IMPL_MATHVEC_DECLARATIONS(VecType)                                              \
    ...
    void Negate() noexcept;                                                                        \
                                                                                                   \
    [[nodiscard]]                                                                                  \
    value_type Dot(const VecType& other) const noexcept;                                           \
                                                                                                   \
    [[nodiscard]]                                                                                  \
    value_type LengthSqr() const noexcept;                                                         \
                                                                                                   \
    [[nodiscard]]                                                                                  \
    value_type Length() const noexcept;                                                            \
                                                                                                   \
    [[nodiscard]]                                                                                  \
    bool IsGreaterThan(const VecType& other) const noexcept;                                       \
                                                                                                   \
    [[nodiscard]]                                                                                  \
    bool IsLessThan(const VecType& other) const noexcept;                                          \
                                                                                                   \
    void Normalize() noexcept;                                                                     \
                                                                                                   \
    [[nodiscard]]                                                                                  \
    bool NearEqual(const VecType& other,                                                           \
                   value_type     tolerance = std::numeric_limits<value_type>::epsilon())          \
        const noexcept;                                                                            \
                                                                                                   \
    [[nodiscard]]                                                                                  \
    bool Within(const VecType& min, const VecType& max) const noexcept;                            \
                                                                                                   \
    ...
    VecType& operator=(value_type val) noexcept;                                                   \
                                                                                                   \
    [[nodiscard]]                                                                                  \
    VecType operator-() const noexcept;                                                            \
                                                                                                   \
    [[nodiscard]]                                                                                  \
    VecType min(const VecType& other) const noexcept;                                              \
                                                                                                   \
    [[nodiscard]]                                                                                  \
    VecType max(const VecType& other) const noexcept;                                              \
                                                                                                   \
    [[nodiscard]]                                                                                  \
    value_type DistanceTo(const VecType& other) const noexcept                                     \
    {                                                                                              \
        return (other - *this).Length();                                                           \
    }                                                                                              \
                                                                                                   \
    [[nodiscard]]                                                                                  \
    value_type DistanceToSqr(const VecType& other) const noexcept                                  \
    {                                                                                              \
    ...
    public:
        ...
        constexpr MVector(const MVector<OtherTy, OtherSize>& other) noexcept
        {
            ...
            {
                auto meit = begin();
                for (auto it = other.begin(), end = it + std::min(other.size(), size()); it != end;
                     ++it, ++meit)
    ...
    public:
        ...
        [[nodiscard]]
        constexpr array_type& get() noexcept
        {
            return m_Data;
        }
        ...
        [[nodiscard]]
        constexpr const OtherTy& To() const noexcept
        {
    ...
    public:
        static inline constexpr Ty One =
            Ty{ value_type(1), value_type(1), value_type(1), value_type(1) };
        static inline constexpr Ty Zero =
            Ty{ value_type(0), value_type(0), value_type(0), value_type(0) };

        static inline constexpr Ty Epsilon =
            Ty{ EpsilonValue, EpsilonValue, EpsilonValue, EpsilonValue };
        static inline constexpr Ty Infinity =
            Ty{ InfinityValue, InfinityValue, InfinityValue, InfinityValue };
        ...
        static inline constexpr Ty Up =
            Ty{ value_type(0), value_type(1), value_type(0), value_type(0) };
        static inline constexpr Ty Down =
            Ty{ value_type(0), value_type(-1), value_type(0), value_type(0) };
        static inline constexpr Ty Left =
            Ty{ value_type(-1), value_type(0), value_type(0), value_type(0) };
        static inline constexpr Ty Right =
            Ty{ value_type(1), value_type(0), value_type(0), value_type(0) };
        static inline constexpr Ty Forward =
            Ty{ value_type(0), value_type(0), value_type(1), value_type(0) };
        static inline constexpr Ty Backward =
            Ty{ value_type(0), value_type(0), value_type(-1), value_type(0) };
    };
}
