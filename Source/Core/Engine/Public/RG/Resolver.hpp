...
namespace Ame::Rg
{
    ...
    public:
        [[nodiscard]]
        const Dg::BufferDesc& GetBufferDesc(const ResourceId& id) const;
        [[nodiscard]]
        const Dg::TextureDesc& GetTextureDesc(const ResourceId& id) const;

    public:
        void           WriteResource(const ResourceId& id);
        void           SetUserData(const ResourceId& id, IObject* userData);
        ResourceViewId WriteBuffer(const ResourceId&             id,
                                   Dg::BIND_FLAGS                bindFlags,
                                   const BufferResourceViewDesc& viewDesc);
        ResourceViewId WriteTexture(const ResourceId&              id,
                                    Dg::BIND_FLAGS                 bindFlags,
    ...
    public:
        ...
        ResourceViewId ReadBuffer(const ResourceId&             id,
                                  Dg::BIND_FLAGS                bindFlags,
                                  const BufferResourceViewDesc& viewDesc);
        ResourceViewId ReadTexture(const ResourceId&              id,
                                   Dg::BIND_FLAGS                 bindFlags,
                                   const TextureResourceViewDesc& viewDesc);

    private:
        ResourceViewId DeclareView(const ResourceId&             id,
                                   Dg::BIND_FLAGS                bindFlags,
                                   const BufferResourceViewDesc& viewDesc);
        ResourceViewId DeclareView(const ResourceId&              id,
                                   Dg::BIND_FLAGS                 bindFlags,
