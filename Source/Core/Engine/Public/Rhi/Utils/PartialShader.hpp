...
namespace Ame::Rhi
{
        ...
        void Initialize(const CombinedShaderCreateDesc& desc)
        {
            ...
            for (auto& curDesc : desc.Shaders)
            {
                ...
                auto& path = paths.emplace_back(
                    std::format("__Composer/{}.aps", static_cast<const void*>(shader)));
                ...
                memorySources.emplace_back(path.c_str(),
                                           shader->Source,
                                           static_cast<uint32_t>(shader->SourceLength));

                CompileFlags(CompileFlags() | shader->CompileFlags);
                LoadConstantBufferReflection(LoadConstantBufferReflection() ||
                                             shader->LoadConstantBufferReflection);
    ...
    private:
                                ...
                                std::span<const Dg::MemoryShaderSourceFileInfo>    memorySources)
        {
            ...
            Dg::CreateMemoryShaderSourceFactory({ memorySources.data(), Count32(memorySources) },
                                                &memoryFactory);
            if (factories.empty())
            {
            ...
            else
            {
                ...
                Dg::CreateCompoundShaderSourceFactory({ factories.data(), Count32(factories) },
                                                      &combinedFactory);
