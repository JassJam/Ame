...
namespace Ame::Scripting
{
    ...
    public:
        ...
        NativeArray(const NativeArray& other)
            : m_Data(std::make_unique<Ty[]>(other.m_Size)), m_Size(other.m_Size)
        {
            std::copy(other.begin(), other.end(), m_Data.get());
        }
        NativeArray(std::span<const Ty> span)
            : m_Data(std::make_unique<Ty[]>(span.size())), m_Size(span.size())
        {
        ...
        NativeArray(Args&&... args)
            : m_Data(std::make_unique<Ty[]>(sizeof...(args))), m_Size(sizeof...(args))
        {
            size_t i = 0;
        ...
        [[nodiscard]]
        NativeSpan<Ty> span() const noexcept
        {
            return NativeSpan<Ty>(m_Data.get(), m_Size);
        }
        [[nodiscard]]
        NativeSpan<Ty> span(size_t offset) const noexcept
        {
            return NativeSpan<Ty>(m_Data.get() + offset, m_Size - offset);
        }
        [[nodiscard]]
        NativeSpan<Ty> span(size_t offset, size_t count) const noexcept
        {
