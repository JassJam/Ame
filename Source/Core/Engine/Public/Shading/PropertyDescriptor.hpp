...
#define AME_DECLARE_MEMBER_SCALAR(FuncName, ImplName, Dims)                                        \
    ...
    {                                                                                              \
        ...
        MaterialPropertyDescriptor& Struct(const String&                     propertyName,
...
#undef AME_DECLARE_MEMBER_SCALAR
    ...
    public:
        ...
        /// <summary>
        /// Get offset of a property in the descriptor for user data, returns InvalidOffset if the
        /// property is not found or if its a resource
        /// </summary>
        [[nodiscard]]
        uint32_t GetOffset(const String& propertyName) const;
    ...
    private:
        ...
        /// </summary>
        [[nodiscard]]
        static uint32_t GetSize(MaterialResourceType type, MaterialDataType dataType, uint8_t dims);

        /// <summary>
        /// Increase the size of the descriptor userdata and return offset for the next property
        /// </summary>
        [[nodiscard]]
        uint32_t AdvanceSize(uint32_t size);
