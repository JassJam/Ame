...
#include <Rhi/Device/DiligentNamespace.h>
...
{
    AME_RHI_DEVICE_VALIDATION_NONE = 0,
    ...
    AME_RHI_DEVICE_VALIDATION_CHECK_SHADER_BUFFER_SIZER = 0x01,
    ...
    /// \remarks  This flag only has effect in Debug/Development builds.
    ///           This type of validation is never performed in Release builds.
    AME_RHI_DEVICE_VALIDATION_COMMITTED_RESOURCE_RELEVANCE = 1 << 0,
...
typedef struct Ame_RhiDeviceCreateDescD3D11_t
{
    DILIGENT_NAMESPACE(Version) GraphicsAPIVersion;
} Ame_RhiDeviceCreateDescD3D11_t;

typedef struct Ame_RhiDeviceCreateDescD3D12_t
{
    DILIGENT_NAMESPACE(Version) GraphicsAPIVersion;
    ...
    /// </summary>
    uint32_t DynamicHeapPageSize; // 0 = Use whatever diligent default deems default.

    /// <summary>
    /// Number of dynamic heap pages that will be reserved by the
    /// global dynamic heap manager to avoid page creation at run time.
    /// </summary>
    uint32_t NumDynamicHeapPagesToReserve; // 0 = Use whatever diligent default deems default.
    ...
    struct
    {
        uint32_t Occlusion;          // 0 = Use whatever diligent default deems default.
        uint32_t BinaryOcclusion;    // 0 = Use whatever diligent default deems default.
        uint32_t Timestamp;          // 0 = Use whatever diligent default deems default.
        uint32_t PipelineStatistics; // 0 = Use whatever diligent default deems default.
        uint32_t Duration;           // 0 = Use whatever diligent default deems default.
    } QueryHeapSizes;
} Ame_RhiDeviceCreateDescD3D12_t;

typedef struct Ame_RhiDeviceCreateDescGL_t
{
} Ame_RhiDeviceCreateDescGL_t;

typedef struct Ame_RhiDeviceCreateDescVulkan_t
{
    /// <summary>
    /// MainDescriptorPool:
    ...
    const char** IgnoredDebugMessageNames;

    void* RequiredDeviceFeatures;
    ...
    /// </summary>
    uint32_t DeviceLocalMemoryPageSize; // 0 = Use whatever diligent default deems default.
    ...
    /// </summary>
    uint32_t HostVisibleMemoryPageSize; // 0 = Use whatever diligent default deems default.
    ...
    /// </summary>
    uint32_t DeviceLocalMemoryReserveSize; // 0 = Use whatever diligent default deems default.
    ...
    /// pages when resources are released.
    /// </summary>
    uint32_t HostVisibleMemoryReserveSize; // 0 = Use whatever diligent default deems default.
...
} Ame_RhiDeviceCreateDescVulkan_t;

typedef enum Ame_RhiDeviceCreateDescType_t
{
    AME_RHI_DEVICE_CREATE_NONE,
    AME_RHI_DEVICE_CREATE_D3D11,
    AME_RHI_DEVICE_CREATE_D3D12,
    AME_RHI_DEVICE_CREATE_GL,
    AME_RHI_DEVICE_CREATE_VULKAN,
} Ame_RhiDeviceCreateDescType_t;

/// <summary>
/// (AME_RHI_DEVICE_CREATE_NONE, nullptr) to mark end of stream
/// </summary>
typedef struct Ame_RhiDeviceCreateDescStream_t
{
    Ame_RhiDeviceCreateDescType_t Type;
    void*                         Desc;
} Ame_RhiDeviceCreateDescStream_t;

//

typedef struct Ame_RhiFullscreenModeDesc_t
{
    struct
    {
    ...
    DILIGENT_NAMESPACE(SCALING_MODE) Scaling;
    DILIGENT_NAMESPACE(SCANLINE_ORDER) ScanlineOrder;

    bool Fullscreen = false;
} Ame_RhiFullscreenModeDesc_t;

typedef struct Ame_RhiSwapchainDesc_t
{
    /// <summary>
    /// Back buffer format. Default value is Diligent::TEX_FORMAT_RGBA8_UNORM_SRGB
    /// </summary>
    DILIGENT_NAMESPACE(TEXTURE_FORMAT) ColorFormat;
    ...
    /// </summary>
    DILIGENT_NAMESPACE(TEXTURE_FORMAT) DepthFormat;

    /// <summary>
    /// Swap chain usage flags. Default value is Diligent::SWAP_CHAIN_USAGE_RENDER_TARGET
    /// </summary>
    DILIGENT_NAMESPACE(SWAP_CHAIN_USAGE_FLAGS) Usage;
    ...
    ///       method.
    /// </summary>
    DILIGENT_NAMESPACE(SURFACE_TRANSFORM) PreTransform;
    ...
    /// </summary>
    bool IsPrimary = true;
} Ame_RhiSwapchainDesc_t;

typedef struct Ame_RhiRenderSurfaceDesc_t
{
    Ame_IWindow_t*              Window; // Window may be shared across multiple devices.
    Ame_RhiFullscreenModeDesc_t FullscreenMode;
    Ame_RhiSwapchainDesc_t      Swapchain;
} Ame_RhiRenderSurfaceDesc_t;

//

typedef struct Ame_RhiDeviceCreateDesc_t
{
    /// <summary>
    /// The device features to use.
    /// </summary>
    DILIGENT_NAMESPACE(DeviceFeatures) Features;

    /// <summary>
    /// The adapter callback to use for the device.
    /// if not set, the first adapter will be used.
    /// </summary>
    Ame_RhiQueryAdapterFn_t AdapterCallback;
    ...
    /// </summary>
    Ame_RhiDeviceCreateDescStream_t* Types;
