...
namespace imcxx
{
    ...
    public:
...
#if _HAS_CXX20
            m_Result._Value = ImGui::DragScalar(impl::get_string(label), type, std::bit_cast<void*>(v), speed,
                                                std::bit_cast<const void*>(&v_min), std::bit_cast<const void*>(&v_max),
                                                format, flags);
#else
            m_Result._Value = ImGui::DragScalar(impl::get_string(label), type, reinterpret_cast<void*>(v), speed,
        ...
        /// </summary>
        template<typename _StrTy, typename _Ty>
        drag(pointer, const _StrTy& label, _Ty* v, size_t size, float speed = 1.f,
             _Ty v_min = (std::numeric_limits<_Ty>::min)(), _Ty v_max = (std::numeric_limits<_Ty>::max)(),
...
#if _HAS_CXX20
            m_Result._Value = ImGui::DragScalarN(impl::get_string(label), type, std::bit_cast<void*>(v), size, speed,
                                                 std::bit_cast<const void*>(&v_min), std::bit_cast<const void*>(&v_max),
                                                 format, flags);
#else
            m_Result._Value = ImGui::DragScalarN(impl::get_string(label), type, reinterpret_cast<void*>(v), size, speed,
                                                 reinterpret_cast<const void*>(&v_min),
                                                 reinterpret_cast<const void*>(&v_max), format, flags);
#endif
        }
        ...
        drag(array<_ViewSize> arr_size, const _StrTy& label, std::array<_Ty, _Size>* v, float speed = 1.f,
             _Ty v_min = (std::numeric_limits<_Ty>::min)(), _Ty v_max = (std::numeric_limits<_Ty>::max)(),
             const char* format = impl::default_c_format<_Ty>, ImGuiSliderFlags flags = 0)
        {
            static constexpr ImGuiDataType type = impl::to_imdatatype<_Ty>();
...
#if _HAS_CXX20
            m_Result._Value =
                ImGui::DragScalarN(impl::get_string(label), type, std::bit_cast<void*>(v->data()), size, speed,
                                   std::bit_cast<void*>(&v_min), std::bit_cast<void*>(&v_max), format, flags);
#else
            m_Result._Value = ImGui::DragScalarN(impl::get_string(label), type, reinterpret_cast<void*>(v->data()),
                                                 size, speed, reinterpret_cast<const void*>(&v_min),
                                                 reinterpret_cast<const void*>(&v_max), format, flags);
#endif
        }
        ...
        /// </summary>
        template<size_t _ViewSize, typename _StrTy, typename _Ty, size_t _Size>
        drag(array<_ViewSize> arr_size, const _StrTy& label, _Ty (&v)[_Size], float speed = 1.f,
             _Ty v_min = (std::numeric_limits<_Ty>::min)(), _Ty v_max = (std::numeric_limits<_Ty>::max)(),
             const char* format = impl::default_c_format<_Ty>, ImGuiSliderFlags flags = 0)
        {
            static constexpr ImGuiDataType type = impl::to_imdatatype<_Ty>();
...
#if _HAS_CXX20
            m_Result._Value =
                ImGui::DragScalarN(impl::get_string(label), type, std::bit_cast<void*>(static_cast<_Ty*>(v)), size,
                                   speed, std::bit_cast<void*>(&v_min), std::bit_cast<void*>(&v_max), format, flags);
#else
            m_Result._Value = ImGui::DragScalarN(impl::get_string(label), type, reinterpret_cast<void*>(v), size, speed,
                                                 reinterpret_cast<const void*>(&v_min),
                                                 reinterpret_cast<const void*>(&v_max), format, flags);
        ...
        drag(array<_ViewSize> arr_size, const _StrTy& label, ImVec2* v, float speed = 1.f,
             float v_min = (std::numeric_limits<float>::min)(), float v_max = (std::numeric_limits<float>::max)(),
        ...
        /// </summary>
        template<size_t _ViewSize, typename _StrTy>
        drag(array<_ViewSize> arr_size, const _StrTy& label, ImVec4* v, float speed = 1.f,
             float v_min = (std::numeric_limits<float>::min)(), float v_max = (std::numeric_limits<float>::max)(),
             const char* format = "%.3f", ImGuiSliderFlags flags = 0) :
#if _HAS_CXX20
