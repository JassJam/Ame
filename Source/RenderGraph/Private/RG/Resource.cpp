...
namespace Ame::Rg
{
    ...
    void ResourceHandle::Import(Dg::IBuffer* resource)
    {
        m_Handle     = BufferHandle{ { Ptr{ resource }, {}, resource->GetDesc() } };
        m_IsImported = true;
    }

    void ResourceHandle::Import(Dg::ITexture* resource)
    {
        m_Handle     = TextureHandle{ Ptr{ resource }, {}, resource->GetDesc() };
        m_IsImported = true;
    }

    void ResourceHandle::SetLocal(Rhi::BufferInitData* initData, const Dg::BufferDesc& desc)
    {
        m_Handle     = BufferHandle{ { {}, Ptr{ initData }, desc } };
        m_IsImported = false;
    }

    void ResourceHandle::SetLocal(Rhi::TextureInitData* initData, const Dg::TextureDesc& desc)
    {
    ...
    size_t ResourceHandle::CreateView(const BufferResourceViewDesc& viewDesc)
    {
    ...
    size_t ResourceHandle::CreateView(const TextureResourceViewDesc& viewDesc)
    {
        size_t viewId = ComputeViewHash(viewDesc);
    ...
    void ResourceHandle::Reallocate(Dg::IRenderDevice* renderDevice)
    {
        std::visit(
            VariantVisitor{ [&](auto& handle)
                            {
                                if (!IsImported())
                                {
    ...
    void ResourceHandle::RecreateViewsIfNeeded(BufferHandle& handle)
    {
    ...
    void ResourceHandle::RecreateViewsIfNeeded(TextureHandle& handle)
    {
        for (auto& viewHandle : handle.Views | std::views::values)
        {
            if (!viewHandle.View) [[unlikely]]
